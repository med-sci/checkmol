...
@click.command()
@click.option('--tekton_features', '-f', help='Tekton result features path')
@click.option('--tekton_target', '-t', help='Tekton result target path')
@click.option('--tekton_experiment', '-e', help='Tekton result experiment path')
def main(tekton_features, tekton_target, tekton_experiment):
    ...
    # emitting results
    logger.info(f"Writing {FEATURES_PATH} to {tekton_features}")
    write_task_result(FEATURES_PATH, tekton_features)

    logger.info(f"Writing {TARGET_PATH} to {tekton_target}")
    write_task_result(TARGET_PATH, tekton_target)

    logger.info(f"Writing {EXPERIMENT_NAME} to {tekton_experiment}")
    write_task_result(EXPERIMENT_NAME, tekton_experiment)
